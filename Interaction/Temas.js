function formatarNome(nome) {
    let nomeAtualizado = nome.replace(/Cilindro/g, "Tema");
    let semExt = nomeAtualizado.replace(/\.[^/.]+$/, '');
...
function tentarCarregarImagem(img, url, tentativas = 15) {
    img.onerror = () => {
        if (tentativas > 0) {
            setTimeout(() => {
...
fetch('https://api.github.com/repos/erikprograming/CatalogoDecoracoes/contents/Imagens/Cilindros')
    .then(res => res.json())
    .then(arquivos => {
        let i = 0;
